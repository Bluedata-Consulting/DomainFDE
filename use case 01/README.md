...
> Then open [`ARCHITECTURE.md`](ARCHITECTURE.md) to see how the parts fit together,
> and [`learner-repo/day-01/HANDS-ON.md`](learner-repo/day-01/HANDS-ON.md) for the
> classroom activity.
...
2. [Fill in your own values first](#2-fill-in-your-own-values-first)
...
## 2. Fill in your own values first

This guide uses placeholders in angle brackets. Write your own values in the
table below before you start, then substitute them wherever you see the
placeholder. Your coach will give you any value you do not have.

| Placeholder | What it is | Example | Your value |
|---|---|---|---|
| `<YOUR_ACCOUNT>` | The Google account you sign in with | `jane.doe@example.com` | |
| `<YOUR_PROJECT_ID>` | The Google Cloud project you will work in | `my-team-agents-dev` | |
| `<YOUR_REGION>` | The region the model is called in | `us-central1` | |
| `<YOUR_MODEL>` | The Gemini model the agents run on | `gemini-2.5-flash` | |
| `<REPO_URL>` | The Git repo holding this kit | `https://github.com/your-org/your-training-repo.git` | |
| `<REPO_NAME>` | The folder that repo creates when cloned | `your-training-repo` | |
...
- **A Google Cloud project you may use,** with billing enabled. That is
  `<YOUR_PROJECT_ID>`.
...
1. Go to **console.cloud.google.com** and sign in as `<YOUR_ACCOUNT>`.
2. At the top of the page, click the project name and choose `<YOUR_PROJECT_ID>`.
...
Paste these two lines, with your own values. The first says which account to work
as, the second says which project to work in.
...
Example, if you were Jane Doe:

```bash
gcloud config set account jane.doe@example.com
gcloud config set project my-team-agents-dev
```
...
cd ~
git clone <REPO_URL>
unzip -o <REPO_NAME>/Day1_HandsOn_ADK_Scaffold.zip -d adlc-kit
...
```bash
REGION=<YOUR_REGION> AGENT_MODEL=<YOUR_MODEL> bash ~/adlc-repo/day-01/setup.sh
```
...
|---|---|---|
| **Ask your admin** | Grant the identity the **Vertex AI User** role: `gcloud projects add-iam-policy-binding <YOUR_PROJECT_ID> --member="serviceAccount:<THE_IDENTITY>" --role="roles/aiplatform.user"` | Best for a class: every machine then works with no sign-in. |
| **Sign in yourself** | `gcloud auth application-default login --no-launch-browser`, follow the link, sign in as `<YOUR_ACCOUNT>`, paste the code back, then run `setup.sh` again. | Quick fix for one machine, if that account already has access. |
...
| **Region** | `<YOUR_REGION>`, for example `us-central1` (set in `agents/.env`) |
| **Model** | `<YOUR_MODEL>`, for example `gemini-2.5-flash` (set in `agents/.env`) |
| **Grant command** | `gcloud projects add-iam-policy-binding <YOUR_PROJECT_ID> --member="serviceAccount:<VM_SERVICE_ACCOUNT>" --role="roles/aiplatform.user"` |
...
1. [`ARCHITECTURE.md`](ARCHITECTURE.md): what talks to what, and where the controls
   will attach later in the programme.
2. [`learner-repo/day-01/HANDS-ON.md`](learner-repo/day-01/HANDS-ON.md): the
   classroom activity.
3. [`learner-repo/day-01/improvement-proposal.md`](learner-repo/day-01/improvement-proposal.md):
