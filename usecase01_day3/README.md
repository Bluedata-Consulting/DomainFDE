...
The code lives in this repository under `usecase01_day3/`. The commands below set one
...
Already cloned on Day 1 or Day 2? `cd ~/domainFDE && git pull` instead, so you get the
Day 3 folder.
...
```bash
echo 'export KIT="$HOME/domainFDE/usecase01_day3"' >> ~/.bashrc
source ~/.bashrc
ls "$KIT/agents"
```

> **Expected:** `complaints_v3_measured`.
...
It writes `usecase01_day3/agents/.env` (your project, region and model) and makes one
...
|---|---|---|
| `No such file or directory` for `usecase01_day3` | Repository cloned before Day 3 was added | `cd ~/domainFDE && git pull`, then `ls "$KIT/agents"` |
...
└── usecase01_day3/                    $KIT
    ...
    ├── .gitignore                     Keeps runs.jsonl and escalations.log out of git
