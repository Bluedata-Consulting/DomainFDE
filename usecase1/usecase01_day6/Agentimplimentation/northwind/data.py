"""Northwind data and meanings, shared by every agent (Day 5: Build).

This is the Day 4 ontology code, moved into one shared package so that the
complaints, returns and billing agents all read the same meanings. Day 5 adds
returns (the supply chain side) to what it can look up.
...
# ---------------------------------------------------------------------------
# 4. RETURNS (Day 5): the supply chain side
# ---------------------------------------------------------------------------
DISPOSITIONS = ["restock", "refurbish", "return_to_vendor", "liquidate", "recycle", "quarantine"]
HIGH_VALUE_GBP = 500
DECISIONS = {"dispositions": [], "claims": [], "approvals": []}   # made while the server runs


def return_view(return_id):
    """Everything about one returned item, found by following links."""
    r = _by_key("returns", "return_id").get(str(return_id).strip().upper())
    if r is None:
        return None
    decided = [d for d in DECISIONS["dispositions"] if d["return_id"] == r["return_id"]]
    return {
        **r,
        "high_value": r["value_gbp"] > HIGH_VALUE_GBP,
        "vendor_claim_open": r["days_to_vendor_deadline"] >= 0,
        "disposition_so_far": decided[-1]["disposition"] if decided else None,
        "allowed_dispositions": ["quarantine"] if r["on_recall_list"] else DISPOSITIONS,
    }


def record_decision(kind, **fields):
    DECISIONS[kind].append(fields)
