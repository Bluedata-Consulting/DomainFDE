...
The code is stored in the repository as `usecase01_day2.zip`. You unpack it once, and
the commands below set one variable, `$KIT`, pointing at the unpacked folder.
...
### 5. Get the repository and unpack the kit

**5a. Download the repository.** First time:
...
Already cloned? Get the latest version instead:
...
**5b. Unpack the Day 2 folder.**

```bash
cd ~/DomainFDE
ls usecase01_day2.zip
python3 -m zipfile -e usecase01_day2.zip .
```

> **Expected:** the first command prints `usecase01_day2.zip`. The last prints nothing
> and creates the folder `usecase01_day2`.

`python3 -m zipfile` is used because Python is always on the VM; `unzip` may not be.
Unpacking again later puts the code back to the version in the zip.

**5c. Set the variable every command below relies on**, and make it stick for new
terminals:

```bash
grep -q 'usecase01_day2' ~/.bashrc || echo 'export KIT="$HOME/DomainFDE/usecase01_day2"' >> ~/.bashrc
source ~/.bashrc
ls "$KIT"
ls "$KIT/agents"
```

> **Expected:** the first `ls` shows `README.md`, `README-training-account.md`,
> `ARCHITECTURE.md`, `usecase1_day2.pdf`, `Day2_Worksheets_UC01.docx`, `setup.sh`,
> `agents` and `tests` (the order depends on your terminal). The second shows
> `complaints_v1_baseline` and `complaints_v2_decision`.
...
|---|---|---|
| `setup.sh: No such file or directory` | The zip was not unpacked | Step 5b, then `ls "$KIT"` |
| `usecase01_day2.zip: No such file` | Repository not downloaded or out of date | Step 5a (`git pull`), then step 5b |
| `$KIT: unbound` or `No such file` | Variable not set in this terminal | `source ~/.bashrc`, then `echo $KIT` |
...
| Test shows `FAIL` without editing | `decision_tree.py` was changed | Undo the change, or unpack the zip again (step 5b) |
...
├── usecase01_day2.zip                 Day 2 kit, as stored in the repository
└── usecase01_day2/                    Created in step 5b.  $KIT
    ├── README.md                      This page
